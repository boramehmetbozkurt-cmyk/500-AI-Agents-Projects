...
def root_license() -> dict[str, Any]:
    ...
    return {
        ...
        "exclusivity_note": "MIT upstream material is not exclusive proprietary IP; OSIRIS-specific value must be separately inventoried.",
    }
...
def spdx_document(packages: list[dict[str, str]], generated_at: str) -> dict[str, Any]:
    ...
    return {
        ...
        "name": "OSIRIS Fusion resolved Python environment",
        "documentNamespace": f"https://osiris.invalid/spdx/{namespace}",
        "creationInfo": {
            "created": generated_at,
            "creators": ["Tool: OSIRIS buyer artifact generator"],
        },
        "packages": [
            {
                "name": item["name"],
...
def build(output: Path) -> dict[str, Path]:
    ...
    asset = {
        "schema": "osiris.asset-manifest.v1",
    ...
    asset_path = output / "osiris-asset-manifest.json"
    sbom_path = output / "osiris-python-sbom.spdx.json"
    rights_path = output / "osiris-data-rights.json"
    hashes_path = output / "osiris-file-hashes.sha256"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate OSIRIS buyer due-diligence artifacts")
