...
BUILTIN_CAPABILITIES: dict[str, CapabilitySpec] = {
    ...
    "direct_reasoning": CapabilitySpec(
        ...
        domain="general",
        description=(
            "Local/fallback model reasoning for non-time-sensitive, non-evidence "
            "tasks such as drafting, rewriting and ideation."
        ),
        kind="local_model",
        keywords=(
            "yaz",
...
def _validate_connector_url(url: str) -> None:
    ...
    raise PermissionError(
        "Connector URL must use HTTPS, except localhost development endpoints"
    )
...
class UniversalToolClient:
    """Dispatch sealed read-only tools without accepting user-supplied URLs."""
    ...
    async def fetch_tool(
        ...
        *,
        ...
        if spec is None or not spec.read_only or not spec.auto_execute:
            raise PermissionError(
                "Capability is not authorized for autonomous read-only execution: "
                f"{tool}"
            )
        if not capability_ready(spec):
            return make_evidence_record(
                ...
                error=(
                    ...
                    f"{spec.url_env or 'required connector settings'}"
                ),
            )
        ...
        if spec.kind == "local_model":
            system = (
                "You are a general-purpose assistant operating as a bounded OSIRIS "
                "Fusion capability. Do not claim fresh facts without evidence. "
                "Perform only the requested drafting, transformation, reasoning or "
                "ideation task."
            )
            ...
            try:
                async with httpx.AsyncClient(
                    ...
                    if len(content) > self.max_response_bytes:
                        raise ValueError(
                            "Connector response exceeds configured evidence limit"
                        )
                    content_type = response.headers.get("content-type", "")
                    if (
                        "json" not in content_type.lower()
                        and content.strip()[:1] not in {b"{", b"["}
                    ):
