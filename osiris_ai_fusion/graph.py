...
async def analyst_node(state: AgentState) -> AgentState:
    settings = get_settings()
    llm = ModelRouter()
    compact = json.dumps(state.get("evidence", {}), ensure_ascii=False)[: settings.max_prompt_evidence_chars]
    system = (
        ...
        "Use only supplied read-only OSINT evidence. Never invent facts. Separate direct observation "
        "from inference, cite the originating tool/source URL for material claims, state uncertainty, "
